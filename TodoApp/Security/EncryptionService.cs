...
public sealed class EncryptionService : IEncryptionService
{
    ...
    {
        // default to 12-byte GCM nonce
        return RandomNumberGenerator.GetBytes(12);
    }

    // AES-GCM (single method shape)
    public byte[] AesGcmEncrypt(byte[] key, byte[] nonce, byte[] plaintext, byte[]? aad = null)
    {
        if (key is null || nonce is null || plaintext is null)
            throw new ArgumentNullException("key/nonce/plaintext must not be null");
        if (key.Length != 16 && key.Length != 24 && key.Length != 32)
            throw new ArgumentException("AES key must be 16/24/32 bytes");
        if (nonce.Length < 12) throw new ArgumentException("GCM nonce should be at least 12 bytes");
    ...
    {
        if (key is null || nonce is null || ciphertextWithTag is null)
            throw new ArgumentNullException("key/nonce/ciphertext must not be null");
        if (key.Length != 16 && key.Length != 24 && key.Length != 32)
            throw new ArgumentException("AES key must be 16/24/32 bytes");
        if (nonce.Length < 12) throw new ArgumentException("GCM nonce should be at least 12 bytes");

        var tagLen = 16;
        if (ciphertextWithTag.Length < tagLen) throw new ArgumentException("ciphertext too short");
    ...
    {
        if (keySize < 2048) throw new ArgumentException("RSA key size must be >= 2048 bits");
    ...
    public byte[] RsaEncrypt(byte[] publicKeySpki, byte[] plaintext, string hashAlg = "SHA256")
    {
        ...
        var padding = ResolveOaep(hashAlg);
    ...
    // Envelope: [4-byte encKeyLen][encKey][12-byte nonce][ciphertext||tag]
    ...
    {
        if (envelope.Length < 4 + 12 + 16) throw new ArgumentException("envelope too short");
        var lenBytes = new byte[4];
        Buffer.BlockCopy(envelope, 0, lenBytes, 0, 4);
        int encKeyLen = System.Net.IPAddress.NetworkToHostOrder(BitConverter.ToInt32(lenBytes, 0));
        if (encKeyLen <= 0 || envelope.Length < 4 + encKeyLen + 12 + 16)
        ...
        try
        {
            return AesGcmDecrypt(aesKey, nonce, ct, aad);
        }
        finally
        {
            Array.Clear(aesKey, 0, aesKey.Length);
    ...
    private static RSAEncryptionPadding ResolveOaep(string hash) => hash.ToUpperInvariant() switch
    {
        ...
        _ => throw new ArgumentException("Unsupported OAEP hash algorithm")
    };
